...
// Dernière mise à jour 4/12/2023
		...
		public static void copieMatrice(int[][] mat1, int[][] mat2){
			...
			for (int i = 0; i < mat1.length; i++) {
				System.arraycopy(mat1[i], 0, mat2[i], 0, mat1[i].length);
			}
		}  // fin copieMatrice
		...
		public static void suppValPoss(int [][] gOrdi, int i, int j, boolean[][][] valPossibles, int [][]nbValPoss){
			...
			for (int colonne = 0; colonne < 9; colonne++) {
				if (gOrdi[i][colonne] == 0) {
					if(valPossibles[i][colonne][valeur]) {
						supprime(valPossibles[i][colonne], valeur);
						nbValPoss[i][colonne]--;
					}
				}
			}
			...
			for (int ligne = 0; ligne < 9; ligne++) {
				if (gOrdi[ligne][j] == 0) {
					if(valPossibles[ligne][j][valeur]) {
						supprime(valPossibles[ligne][j], valeur);
						nbValPoss[ligne][j]--;
					}
				}
			}
			...
			for (int ligne = carreLigne; ligne < carreLigne + 3; ligne++) {
				for (int colonne = carreColonne; colonne < carreColonne + 3; colonne++) {
					if (gOrdi[ligne][colonne] == 0) {
						if(valPossibles[ligne][colonne][valeur]) {
							supprime(valPossibles[ligne][colonne],valeur);
		...
		public static int initPartie(int [][] gSecret, int [][] gHumain, int [][] gOrdi, boolean[][][] valPossibles, int [][]nbValPoss){
			...
			System.out.println("Veuillez saisir la grille que vous voulez faire : 1 | 2 | 3 | 4");
			int grille = saisirEntierMinMax(1,4);
			saisirGrilleIncompleteFichier(nbTrou,gOrdi, "grille"+ grille + ".txt");
			initGrilleIncomplete(nbTrou,gOrdi,gOrdi);
		...
		public static int tourOrdinateur(int [][] gOrdi, boolean[][][] valPossibles, int [][]nbValPoss){
			...
			//Ut.afficher(nbValPoss[trou[0]][trou[1]]);
			//Ut.afficher(valPossibles[trou[0]][trou[1]]);

			if( nbValPoss[trou[0]][trou[1]] == 1){
				gOrdi[trou[0]+1][trou[1]+1] = uneValeur(valPossibles[trou[0]][trou[1]]);
				System.out.println("l'ordi a joué");
				return nbPenalitee;
			...
			if(nbValPoss[trou[0]][trou[1]] <= 3) {
				...
				System.out.println("L'ordinateur essaye cette valeur "  + gOrdi[trou[0]][trou[1]] +  " pour l'emplacement " + "[" + (trou[0] + 1) + "," + (trou[1] + 1) + "]");
				System.out.println("est ce que cette valeur est juste ?  oui ou non ");
				String reponse = Ut.saisirChaine();

				while (reponse.equals("non")){
					nbPenalitee ++;
					supprime(valPossibles[trou[0]][trou[1]],gOrdi[trou[0]][trou[1]]);
				...
				nbPenalitee ++;
				Ut.afficherSL("L'ordinateur a choisis de prendre un joker, veuillez lui renseigner la case: " + "[" + (trou[0] + 1) + "," + (trou[1] + 1) + "]");
		...
		public static void rotationGrille (int[][] g){

			int taille = g.length;
			int [][] grilleRotation = new int[taille][taille];

			for(int i = 0; i < taille;i++){
				for(int j = 0;j<taille;j++){
					grilleRotation[i][j] = g[taille - j - 1][i];
				}
			}
			copieMatrice(grilleRotation, g);
		}

		public static void symetrieHorizontale(int[][] g) {
			int taille = g.length;
			int[][] grilleSymetrique = new int[taille][taille];

			for (int i = 0; i < taille; i++) {
				for (int j = 0; j < taille; j++) {
					grilleSymetrique[i][j] = g[taille - i - 1][j];
				}
			}
			copieMatrice(grilleSymetrique, g);
		}

		public static void symetrieDiag(int[][] g) {
			int taille = g.length;
			int[][] grilleSymertieDiag = new int[taille][taille];

			for (int i = 0; i < taille; i++) {
				for (int j = 0; j < taille; j++) {
					grilleSymertieDiag[i][j] = g[j][i];
				}
			}
			copieMatrice(grilleSymertieDiag, g);

		}

		public static void echange2Ligne(int[][] g, int ligne1, int ligne2) {
			int taille = g.length;

			for (int i = 0; i < taille; i++) {
				int temp = g[ligne1][i];
				g[ligne1][i] = g[ligne2][i];
				g[ligne2][i] = temp;
			}
		}

		public static void constructionGrille(int [][] g){

			for(int i = 0;i<50;i++){

				int rand = Ut.randomMinMax(0,3);

				if(rand == 0){
					rotationGrille(g);
				}
				else if (rand == 1) {
					symetrieHorizontale(g);
				}
				else if (rand == 2){
					symetrieDiag(g);
				}
					...
					int taille = g.length;
					int ligne1 = Ut.randomMinMax(0,taille-1);
					int ligne2;
					do{
						ligne2 = Ut.randomMinMax(0,taille-1);
					}while (ligne1 == ligne2);

					echange2Ligne(g,ligne1,ligne2);
				}
