...
public class SudokuBase {
	...
	public static void main(String[] args) {
		...
		//copieMatrice(grille);
		// ensPlein(10);
		//supprime(tableauplein, 7);
		//uneValeur(tableauplein);
		afficheGrille(3,grille2);
	...
	public static int saisirEntierMinMax(int min, int max) {
		...
		int nb;
		do {
			System.out.println("Veuillez saisir un nombre compris entre le min et le max : ");
			nb = Ut.saisirEntier();
		}
		while (nb <= min || nb >= max);
		return nb;
	...
	public static int uneValeur(boolean[] ens){
		int valeuraupif = Ut.randomMinMax(1,ens.length-1);
		Ut.afficher(valeuraupif);
		return valeuraupif;
    }
    ...
    public static void afficheGrille(int k,int[][] g){
	    ...
	    for (int i = 0; i < dim; i++) {
		    if (i % g.length == 0 && i > 0) {
			    for (int l = 0; l < dim * 2 + k - 1; l++) {
				    System.out.print("-");
			    }
			    System.out.println();
		    }

		    System.out.print((i % g.length + 1) + " ");

		    for (int j = 0; j < dim; j++) {
			    System.out.print(g[i % g.length][j % g.length] + " ");
			    if (j % g.length == g.length - 1 && j < dim - 1) {
				    System.out.print("| ");
			    }
		    }
		    System.out.println();
